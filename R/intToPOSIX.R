...
#' 
#' Takes integer value produced by `as.numeric(x)`, where `x` is a
...
#' PSIXct vector
#' @param tz Time zone of the vector (see [as.POSIXct()]).
#' @return POSIXct vector
#' @note There is no check that as.numeric applied to a POSIX vector produced
#' `timeVector`.  So, caution is required in using this function. It was
